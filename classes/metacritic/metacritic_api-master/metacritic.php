...
class MetacriticAPI
{
    ...
    public function get_metacritic_page($game_name)
    {
        ...
        # Get the webpage
        $response = Unirest\Request::get("http://www.metacritic.com/game/playstation-3/" . $game_name, $headers = array(), $parameters = null);                            
    ...
    public function get_metacritic_scores()
    {
		...
		if(!$error)
		{
		    ...
		    foreach($html->find('span[itemprop=datePublished]') as $element) 
		    {
			 ...
			 foreach($html->find('span[itemprop=description]') as $element) 
		    {
		        $description = trim($element->plaintext);
		    }
